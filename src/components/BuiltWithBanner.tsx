...
export const BuiltWithBanner = () => {
  ...
  const ToolItem = ({ tool, size = "featured" }: { tool: Tool; size?: "featured" | "supporting" }) => {
    const isFeatured = size === "featured";
    const iconSize = isFeatured ? "w-10 h-10 md:w-12 md:h-12" : "w-9 h-9 md:w-10 md:h-10";
          ...
          <TooltipTrigger asChild>
            <a href={tool.url} target="_blank" rel="noopener noreferrer" className="inline-block">
              <div className="flex flex-col items-center gap-2 cursor-pointer group">
                ...
                <span
                  className={`text-xs font-medium text-muted-foreground transition-colors duration-200 group-hover:text-foreground ${
